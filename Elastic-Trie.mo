...
module {
      ...
      switch t {
        ...
        case (#leaf(l)) {
          let len = List.size(l.keyvals);
            len <= MAX_LEAF_SIZE + 1 // Compatible with a bug from the previous version
          and
            len == l.size
          and
            List.all(
  ...
  /// Convert Trie to AssocList    -- This function is customized by @ICLighthouse
  public func toList<K, V>(t : Trie<K, V>) : AssocList<Key<K>, V>{
      switch t {
          case(#empty){ null };
          case(#leaf(l)){ l.keyvals };
          case(#branch(b)){
            List.append(toList(b.left), toList(b.right))
          };
      };
  };
  // Shrink trie    -- This function is customized by @ICLighthouse
  public func shrink<K, V>(t : Trie<K, V>) : Trie<K, V>{
      switch t {
          case(#empty){ t };
          case(#leaf(l)){ t };
          case(#branch(b)){
            if (size(t) <= MAX_LEAF_SIZE){
              let kvs = toList(t);
              #leaf({ size = List.size(kvs); keyvals = kvs })
            }else{
              branch(shrink(b.left), shrink(b.right))
            };
          };
      };
  };
        ...
        switch t {
          ...
          case (#branch(b)) {
            ...
            if (not bit) {
              ...
              if (size(l) <= MAX_LEAF_SIZE){
                (branch(shrink(l), b.right), v_)
              }else{
                (branch(l, b.right), v_)
              };
            }
